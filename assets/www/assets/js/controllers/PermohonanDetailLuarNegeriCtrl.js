app.controller('PermohonanDetailLuarNegeriCtrl', function ($compile, $ionicHistory, $templateCache, $scope, $rootScope, $stateParams, $state, $ionicLoading, _Sweet, _store, _Sweet, _MRZ, _booking, _wilayah, _toastCtrl) {
    ...
    try {
      var param = [],
        fnSuccess = function (succ) {
          ...
          } else if (succ.data.SUCCESS == "false") {
            if (succ.data.CODE = 23505) {
              _Sweet('Error', 'NIK telah terdaftar', 'error');
            } else {
              _Sweet('Error', 'Gagal terhubung dengan server', 'error');
            }
          } else {
        ...
        fnError = function (err) {
          _Sweet('Mohon Maaf', 'Terjadi Kesalahan Cek NIK Atau Koneksi Internet Anda', 'error');
          $ionicLoading.hide();
        }
      for (var i = 0; i < $scope.formPermohonan.pemohon.length; i++) {
      ...
      param.ADMIN_AREA_ID = param.admin_area_id;
      param.CITY_ID = param.city_id;
      param.DISTRICTS_ID = param.district_id;
