app.controller('PermohonanCtrl', function ($scope, $compile, $ionicHistory, $rootScope, $timeout, $state, $stateParams, _datePicker, _http, _store, _booking, _Sweet) {
  ...
  $scope.pushDate = function (key, data) {
    for (var i in data) {
      if (key == "aDate_AVAILABLE"){
        if (Number(data[i].AVAIL_TOTAL) > 0){
  ...
  $scope.checkTanggalKuota = function(callback){
    var selected;
    for (var i = 0; i < $scope.Date_AVAILABLE.length; i++) {
      var temp = $scope.Date_AVAILABLE[i]
      if (temp.DATE == $scope.form.tanggal){
        selected = temp;
        break;
      }
    }
    if ($scope.form.tanggal){
      if ($scope.form.jumlah > selected.AVAIL_TOTAL){
        _Sweet({
          title: "PERHATIAN",
          text: sprintf("JUMLAH PEMOHON TIDAK BOLEH MELEBIHI DARI TOTAL KUOTA TERSEDIA\n( TOTAL: %s )", selected.AVAIL_TOTAL)
        });
      }else{
        if (typeof callback == "function"){
          if (["Pagi", "Siang"].Contains($scope.form.waktu)){
            callback();
          }else{
            _Sweet({
              title: "PERHATIAN",
              text: "SILAHKAN PILIH WAKTU KEDATANGAN"
            }).then($scope.pilihTanggal);
          }
        }
      }
    }else{
      if (callback){
        _Sweet({
          title: "PERHATIAN",
          text: "SILAHKAN PILIH TANGGAL DENGAN KUOTA TERSEDIA"
        }).then($scope.pilihTanggal);
      }
    }
  }

  $scope.changeJumlahPemohon = function(){
    $scope.checkTanggalKuota();
  }
    ...
    _http.post(config.api + "/layanan/wni/quota/info", {
      ...
      if (response.data.SUCCESS && response.data.QUOTA.length > 0) {
        ...
        if (response.data.CODE == "404") {
          _Sweet({
            title: 'Mohon Maaf',
            text: 'Pilih Tanggal & Waktu Kedatangan Dengan Kuota Tersedia',
            type: 'warning',
            confirmButtonText: 'OK',
          }).then(function (succ) {
            $scope.pilihTanggal();
  ...
  $scope.PermohonanDetail = function () {
    $scope.checkTanggalKuota(function(){
      ...
      try {
        ...
        } else if ($scope.form.jumlah > $scope.quotaSedia.AVAILABILITY) {
          _Sweet("Mohon Maaf", "Qouta Tidak Tersedia Untuk " + $scope.form.jumlah + " Pemohon, Silahkan Pilih Tanggal / Waktu Kedatangan Lain", "warning");
        } else {
          if ($.inArray(tanggal, _datePicker.dateCustom.aDate_AVAILABLE) >= 0) {
            if ($scope.form.waktu == "") {
              ...
              _Sweet({
                ...
                $state.go('menu.permohonandetail', {
